...
locals {
  ...
  # nr of months where only one of the two secrets is valid
  one_valid = var.validity - 2 * var.overlap
...
resource "time_rotating" "green" {
  ...
  rfc3339         = timeadd(time_rotating.blue.rotation_rfc3339, "${(-var.overlap - local.one_valid) * local.units[var.unit].timeadd_factor }${local.units[var.unit].timeadd_unit}")
  rotation_minutes = time_rotating.blue.rotation_minutes
  rotation_hours = time_rotating.blue.rotation_hours
  rotation_months = time_rotating.blue.rotation_months
